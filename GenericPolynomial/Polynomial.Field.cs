	...
	public partial class Polynomial<T>
	{
		public static class Field
		{
			...
			public static Polynomial<T> ExponentiateMod(Polynomial<T> startPoly, T exponent, Polynomial<T> polynomialModulus, T integerModulus)
			{
				...
				while (i < t)
				{
					A = Field.ModMod(Polynomial<T>.Square(A), polynomialModulus, integerModulus);
					if (bitArray[i] == true)
					{
						result = Field.ModMod(Polynomial<T>.Multiply(A, result), polynomialModulus, integerModulus);
					}
					i++;
				}
			...
			public static Polynomial<T> ModPow(Polynomial<T> poly, T exponent, Polynomial<T> mod)
			{
				...
				{
					return Polynomial<T>.Field.Modulus(Polynomial<T>.Square(poly), mod);
				}
				...
				while (GenericArithmetic<T>.GreaterThan(exponent, GenericArithmetic<T>.Zero))
				{
					...
					{
						result = Polynomial<T>.Field.Modulus(Polynomial<T>.Multiply(result, @base), mod);
					...
					@base = Polynomial<T>.Field.Modulus(Polynomial<T>.Square(@base), mod);
					exp = GenericArithmetic<T>.Divide(exp, GenericArithmetic<T>.Two);
				}

				return Polynomial<T>.Field.Modulus(result, mod);
				...
				while (GenericArithmetic<T>.NotEqual(counter, GenericArithmetic<T>.Zero)) {	total = Polynomial<T>.Multiply(total, poly);
					if (total.CompareTo(mod) < 0) { total = Field.Modulus(total, mod); }	counter = GenericArithmetic<T>.Decrement(counter);
				} return total;	*/
			}
			...
			public static bool IsIrreducibleOverField(Polynomial<T> f, T p)
			{
				...
				Polynomial<T> reducedField = Field.ModMod(splittingField, f, p);
