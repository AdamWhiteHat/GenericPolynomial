...
namespace TestGenericPolynomial
{
	...
	public class TypeArithmetic<T>
	{
		...
		[Test]
		[TestCase("2981", "8")]
		public virtual void Ln(string argument, object expectedString)
		{
			T input = GenericArithmetic<T>.Parse(argument);
			T expected = GenericArithmetic<T>.Parse(expectedString.ToString());
			...
			//Assert.AreEqual(expected, actual);
			Assert.That(actual, Is.EqualTo(expected).Within(2), null, null);
		}
		...
		public virtual void Power(string argument, int exponent, string expected)
		{
			...
			string actual = GenericArithmetic<T>.PowerInt(input, exponent).ToString();
